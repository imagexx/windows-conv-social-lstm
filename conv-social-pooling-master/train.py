...
# Initialize network
def train_model():  ##winsows 里面要运行多线程需要if __name__ == '__main__':来保护
    net = highwayNet(args)
    if args['use_cuda']:
        net = net.cuda()
    ...
    trSet = ngsimDataset('F:/数据集/NGSIM-txt/TrainSet.mat')
    valSet = ngsimDataset('F:/数据集/NGSIM-txt/ValSet.mat')
    trDataloader = DataLoader(trSet,batch_size=batch_size,shuffle=True,num_workers=2,collate_fn=trSet.collate_fn)
    valDataloader = DataLoader(valSet,batch_size=batch_size,shuffle=True,num_workers=2,collate_fn=valSet.collate_fn)    ### 并行处理的工作数量
    ...
    torch.save(net.state_dict(), 'F:/数据集/NGSIM-txt/models')


if __name__ == '__main__':  ##winsows 里面要运行多线程需要if __name__ == '__main__':来保护
    train_model()
